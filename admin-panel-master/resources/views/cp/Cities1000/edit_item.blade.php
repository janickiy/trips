...
<div class="form-group">
  ...
  <div class="col-sm-8"><input autocomplete="off" type="text" name="name" value="{{$item->name}}" placeholder="{{__('cities_1000.name')}}" class="form-control input-sm" id="column_name" required ></div>
</div>
...
<div class="form-group">
    ...
    <div class="col-sm-8">
        <input type="text" autocomplete="off" name="name_ru" value="{{$item->name_ru}}" placeholder="{{__('cities_1000.name_ru')}}" class="form-control input-sm" id="column_name_ru">
</div>
</div>
...
<div class="box-header ui-sortable-handle">
    ...
    <!--- Essense:  --->
    <h3 class="box-title">Страна и регион</h3>
...
<div class="form-group">
    ...
    <div class="col-sm-4">
        <label for="" class="control-label">Код страны</label>
    </div>
    
    <div class="col-sm-4">
        <input type="text" name="country_code" value="{{$item->country_code}}" placeholder="Код страны" class="form-control input-sm" id="column_country_code">
    </div>
    <div class="col-sm-4">
        @if($item->country_code != null) <span style="display: inline-block; padding-top: 7px;">
...
<div class="form-group">
    ...
    <div class="col-sm-4">
        <label for="" class="control-label">ID региона</label>
    </div>
    
    <div class="col-sm-4">
        <input type="text" name="region_id" value="{{$item->region_id}}" placeholder="ID региона" class="form-control input-sm" id="column_region_id">
    </div>
    <div class="col-sm-4">
        @if($item->region_id > 0) <span style="display: inline-block; padding-top: 7px;">
        {{$item->countryRegion['name_ru']}}
        </span> @endif
    </div>
...
Для указания страны введите код страны из раздела <a href="{{URL::to('/')}}/cp/country">Страны</a>, а для указания региона введите ID региона из раздела <a href="{{URL::to('/')}}/cp/region">Регионы</a> и нажмите "Сохранить".
</div>
...
@if($item->wiki_entity != null)<span style="display: inline-block; padding-top: 7px;"><a href="https://www.wikidata.org/wiki/{{$item->wiki_entity}}" target="_blank">{{$item->wiki_entity}}</a></span>@else <span style="display: inline-block; padding-top: 7px;">Пример: Q64</span> @endif 
    ...
    <div class="col-sm-4">
<span class="btn btn-sm btn-default btn-flat" id="download_from_wikidata">Загрузить из WikiData</span>
<span class="" id="downloading_from_wikidata" style="display: none;">Загружаю... <i class="fa fa-fw fa-spinner"></i></span>
<span class="" id="downloading_from_wikidata_error" style="display: none;">Произошла ошибка! <i class="fa fa-fw fa-remove"></i></span>
<span class="" id="downloading_from_wikidata_success" style="display: none;">Готово! <i class="fa fa-fw fa-check-square-o"></i></span>
    </div>
...
@if($item->has_photo == 1)
    <div class="box-body">
        <p class="text-center" id="preview_exists">
            <img src="{{$imageUrl}}" style="max-height: 250px;" id="preview_photo">
        </p>
    </div>
@else
    <div class="box-body">
        <p class="text-center" id="preview_not_exists">Нет фото.</p>
    </div>
@endif    
    ...
    <div class="box-footer">
        ...
        <a href="#" class="btn btn-sm btn-danger btn-flat delete-photo-btn" city_id="{{$item->id}}">Удалить фото</a>
...
<div style="display: none;">
    <input type="file" name="photo" id="input_photo_city_id_{{$item->id}}" accept=".jpg, .jpeg" class="input_photo" city_id="{{$item->id}}">
</div>
...
/**
 *  Загрузка данных о городе из WikiData
 */
$('body').on('click', '#download_from_wikidata', function() {

    
    let cityID = $('#column_id').val();
    let wikiEntity = $('#column_wiki_entity').val();
    
    if(wikiEntity.length > 0) {
        console.log("column_wiki_entit", "api/parse_wikidata_by_q_entity?update=yes&q=" + wikiEntity, cityID);
        
        $('#download_from_wikidata').hide();
        $('#downloading_from_wikidata').show();
        
        $.ajax({
            type: 'get',
            url: "/api/parse_wikidata_by_q_entity?update=yes&q=" + wikiEntity + "&city_id=" + cityID,
        })
        .done (function (response) {
            $('#downloading_from_wikidata').hide();
            $('#downloading_from_wikidata_success').show();

        }).fail(function (response) {
            $('#downloading_from_wikidata').hide();
            $('#downloading_from_wikidata_error').show();
