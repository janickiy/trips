...
@section('content_title') Парсинг городов из WikiData @endsection
...
function getCity() {
    ...
    .done (function (response, textStatus, xhr) { 
        ...
        if(response.currentcity != null) {
            ...
            parseCity(response.currentcity.id);
        } else {
            $('#start').toggle();
            $('#start-spinner').toggle();
            $('#working').toggle();  
...
function parseCity(id) {
    $.ajax({
        type: 'get',
        url: '{{URL::to('/')}}/api/find_by_geonames_id/' + id
    })
    .done (function (response, textStatus, xhr) { 
...
function showLog(obj) {
    ...
    $('#log-table').prepend(`
        <tr class="log-row">
            <td>` + obj.id + `</td>
            <td><a href="{{URL::to('/')}}/cp/cities_1000/edit/` + obj.city.id + `" target="_blank">` + obj.city.name + `</a></td>
