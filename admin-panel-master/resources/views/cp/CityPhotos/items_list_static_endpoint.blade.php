...
Сервер изображений: {{env('STATIC_ENDPOINT')}}
@endsection
...
<div class="one_good_image_block bg-light">
    <img src="@if($item->has_photo == 1){{$endpoint}}/pics/{{$item->id}}.jpg{{''}}@else{{$endpoint}}{{'/images/no-city-photo.jpg'}}@endif" class="city_photo one_good_image rounded" id="photo_id_">
</div>
        
            <ul class="products-list product-list-in-box text-left">
            <!--
                <li class="item text-center" style="overflow: hidden;">
                
<div class="city_photo_box">
    <div class="city_photo_inner_box">
    <img src="@if($item->has_photo == 1){{URL::to('/')}}/storage/city_photos/{{$item->id}}.jpg{{''}}@else{{URL::to('/')}}{{'/storage/default_photos/no-city-photo.jpg'}}@endif" class="city_photo" id="photo_id_">
    </div>
</div>
                </li>-->
                <li class="item">
...
<form method="post" action="{{URL::to("/")}}/upload_city_photo" autocomplete="off" enctype="multipart/form-data" id="form_id_{{$item->id}}">
{{ csrf_field() }}
