...
@section('content_title') Города из GeoNames @endsection

@section('content_breadcrumb')
<ol class="breadcrumb">
    <li><a href="{{URL::to("/")}}/cp">{{ __('cp.control_panel') }}</a></li>
    <li class="active">Города из GeoNames:</li>
</ol>
@endsection

@section('content_description')
Здесь отображаются города, скачанные из GeoNames. Только просмотр, нет редактирования. 
@endsection
    ...
    <form method="get" action="" autocomplete="off" id="search_cities_1000">
     <p>
        <div class="input-group">
            <input placeholder="Поиск городов из GeoNames" type="text" name="q" class="form-control" value="{{ app('request')->input('q') }}">
            <span class="input-group-btn">
            <button type="submit" id="search-btn" class="btn btn-success">
                <i class="fa fa-search"></i>
            </button>
            <a href="{{URL::to('/')}}/cp/geonames_cities" id="cancel-search-btn" class="btn btn-danger">
...
@if(app('request')->has('population_sort'))
    @if(app('request')->input('population_sort') == 'asc')
        <a href="{{URL::to('/')}}/cp/geonames_cities?q={{app('request')->input('q')}}&population_sort=desc"><i class="fa fa-fw fa-sort-amount-asc" title="По возрастанию"></i></a>
    @elseif (app('request')->input('population_sort') == 'desc')
        <a href="{{URL::to('/')}}/cp/geonames_cities?q={{app('request')->input('q')}}&population_sort=asc"><i class="fa fa-fw fa-sort-amount-desc" title="По убыванию"></i></a>
    @endif
@else 
    <a href="{{URL::to('/')}}/cp/geonames_cities?q={{app('request')->input('q')}}&population_sort=asc" style="color: #000;"><i class="fa fa-fw fa-sort-amount-desc" title="По убыванию"></i></a>
@endif
...
<td><a href="{{URL::to("/")}}/cp/geonames_cities/edit/{{$item->id}}"><i class="fa fa-fw fa-eye"></i></a></td>
</tr>
@empty
